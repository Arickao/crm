	...
	<script>
		$(function () {
			...
			$("#create-transactionStage").change(function () {
				...
				/*
				* 目标: 填写可能性
				* 	   阶段有了stage,阶段和可能性之间的对应关系pMap,但是pMap是java语言中的键值对关系( java中的map对象 )
				*      首先将pMap使用 json 转换为js中的键值对关系
				* */

				//接下来取  (可能性)
				//不能以json.key的形式来取value，因为stage是一个可变的变量，如果是这样的key，不能以传统的json.key的形式来取值
				//要使用的取值方式为json[key]
			...
			$("#search-Activity").keydown(function (event) {
				if (event.keyCode == 13){
					$.ajax({
						url:"workbench/activity/getActivityLikeName",
						data:{
						...
						success:function (data) {
							...
							$.each(data,function (i,n) {
								html += '<tr>';
								html += '<td><input type="radio" name="activity" value="'+n.id+'" onchange="editActivity(\''+n.id+'\',\''+n.name+'\')"/></td>';
								html += '<td>'+n.name+'</td>';
			...
			$("#search-findContacts").keydown(function (event) {
				if (event.keyCode == 13){
					$.ajax({
						url:"workbench/contacts/getContactsLikeName",
						data:{
							"fullname":$.trim($("#search-findContacts").val())
						},
						type:"get",
						dataType:"json",
						success:function (data) {
							...
							$.each(data,function (i,n) {
								html += '<tr>';
								html += '<td><input type="radio" name="contacts" value="'+n.id+'" onchange="editContacts(\''+n.id+'\',\''+n.fullname+'\')"/></td>';
			...
			$("#saveBtn").click(function () {
				var t1 = $.trim($("#create-transactionName").val());
				var t2 = $.trim($("#create-expectedClosingDate").val());
				var t3 = $.trim($("#create-accountName").val());
				var t4 = $.trim($("#create-transactionStage").val());
				if (t1 == "" || t2 == "" || t3 == "" || t4  == ""){
					alert("必填项目不能为空");
					return false;
				}
		...
		function editContacts(id,fullname){
...
<body>
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<div class="btn-group" style="position: relative; top: 18%; left: 8px;">
						<form class="form-inline" role="form">
						  <div class="form-group has-feedback">
						    <input type="text" class="form-control" style="width: 300px;" placeholder="请输入联系人名称，支持模糊查询" id="search-findContacts">
						    <span class="glyphicon glyphicon-search form-control-feedback"></span>
						  </div>
						</form>
					</div>
	...
	<form id="tranFrom" action="workbench/transaction/save" method="post" class="form-horizontal" role="form" style="position: relative; top: -30px;">
		...
		<div class="form-group">
			...
			<div class="col-sm-10" style="width: 300px;">
				...
				<input type="hidden" id="hidden-contactsName" name="contactsId">
			</div>
		</div>
