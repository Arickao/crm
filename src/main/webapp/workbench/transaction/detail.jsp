...
<%@ page import="org.example.crm.setting.domain.DicValue" %>
<%@ page import="java.util.Map" %>
<%@ page import="java.util.Set" %>
<%@ page import="org.example.crm.workbench.domain.Tran" %>
...
<script type="text/javascript">
	...
	function showHistoryList(){
		$.ajax({
			url : "workbench/transaction/getHistoryListByTranId",
			dataType : "json",
			data : {
				"tranId" : "${t.id}"
			},
	...
	function changeStage(stage,i){
		...
		$.ajax({
			...
			type : "post",
			success : function (data){
				//data : [{success:true},{"tran":{交易}}]
				if (data.success){
					//1.改变阶段成功后，需要在详细信息页上局部刷新 阶段、可能性、修改人以及修改时间
	...
	function changeIcon(stage,index1){
		...
		if (currentPossibility == "0"){
			...
			//如果当前阶段的可能性步为0，前7个有可能是绿圈、绿色标记、黑圈，后两个一定是黑叉
...
<body>
	...
	<div style="position: relative; left: 40px; top: -50px;">
		...
		<span id="<%=i%>" onclick="changeStage('<%=listStage%>','<%=i%>')" class="glyphicon glyphicon-remove mystage"
			  ...
			  data-content="<%=dicValue.getText()%>" style="color: #FF0000;">
							</span>
		-----------
		...
		<span id="<%=i%>" onclick="changeStage('<%=listStage%>','<%=i%>')" class="glyphicon glyphicon-remove mystage"
			  data-toggle="popover" data-placement="bottom"
			  data-content="<%=dicValue.getText()%>" style="color: #000000;">
							</span>
		-----------
		...
		<%
			...
			//2.如果当前阶段的可能性不为0，前7个有可能是绿圈，绿色标记，黑圈，后两个一定是黑X
			...
			for (int i=0; i<dvList.size(); i++) {
				...
				//如果遍历出来的阶段的可能性为0，一定是黑叉，说明是后两个阶段
				if ("0".equals(listPossibility)){
		%>
		...
		<%
			...
			//如果遍历出来的阶段可能性不为0，说明是前7个阶段，绿圈，绿色标记，黑圈
			//如果是当前阶段，是绿色标记
			if (i==index){
		...
		<span id="<%=i%>" onclick="changeStage('<%=listStage%>','<%=i%>')" class="glyphicon glyphicon-map-marker mystage"
			  data-toggle="popover" data-placement="bottom"
			  data-content="<%=dicValue.getText()%>" style="color: #90F790;">
						</span>
		-----------
	...
	<%--<span class="glyphicon glyphicon-ok-circle mystage" data-toggle="popover" data-placement="bottom" data-content="资质审查" style="color: #90F790;"></span>
		...
		-------------%>
		<span class="closingDate">${t.nextContactTime}</span>
	</div>
